...
while True:
    ...
    def r2_make_move(game_info: 'GameInfo'):
        ...
        if team_size != 5:
            ...
            rank_range = len(tier1_ranks)
            shop_pet_ranking = create_range_list(rank_range, tier1_ranks, game_info, game_info.player_info.shop_pets)
            print("-------------Shop pet ranking-------------", flush = True)
            print(shop_pet_ranking)
            ...
            # once you have 3 top-3 pets, use up your coins
                ...
                if game_info.player_info.coins >= 3:
                    ...
                    if len(game_info.player_info.shop_foods) > 0:
                        rank_range = len(t2_food_ranks)
                        shop_food_ranking = create_range_list(rank_range, t2_food_ranks, game_info, game_info.player_info.shop_foods)
                        if (team_pet_feed(game_info, shop_food_ranking[0]) == 0):
                            return
    ...
    def team_pet_feed(game_info, shop_food_index):
        min_pet_score = 300
        min_pet_index = None

        if (game_info.player_info.shop_foods[shop_food_index] is None):
            print("ERROR: invalid food index", flush = True)
            return game_info.player_info.coins

        # check if the coins are sufficient 
        if (game_info.player_info.coins >= 3):

            # for each of the pets in team
            for pet_index, team_pet in enumerate(game_info.player_info.pets):
                if team_pet is not None:
                    # calculate pet score
                    pet_score = team_pet.level * (team_pet.health + team_pet.attack)

                    if pet_score < min_pet_score:
                        min_pet_score = pet_score
                        min_pet_index = pet_index

            # if min_pet_index is not empty
            if min_pet_index is not None:
                ...
                print("ERROR: No such pet calulated", flush = True)
                return game_info.player_info.coins
        else:
    ...
    def make_move(game_info: 'GameInfo'):
        ...
        if (game_info.round_num == 1):
            ...
            print(f"Round {game_info.round_num}:", flush = True)
            r5_make_move(game_info, tier3_ranks, t3_food_ranks, t3_food_dict, game_info.round_num)
            print(f"r{game_info.round_num} move performed", flush = True)
            return

        elif (game_info.round_num >= 7):
            print(f"Round {game_info.round_num}:", flush = True)
            r5_make_move(game_info, tier4_ranks, t4_food_ranks, t4_food_dict, game_info.round_num)
            print(f"r{game_info.round_num} move performed", flush = True)
            return

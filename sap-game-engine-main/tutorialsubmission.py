...
while True:
    ...
    def r2_make_move(game_info: 'GameInfo'):
        ...
        # in future, reroll
        if game_info.player_info.coins < 3:
            # if (order_pets(game_info, game_info.player_info.pets)) == 0:
            #     pass
            order_pets(game_info, game_info.player_info.pets)
    ...
    def r3_make_move(game_info: 'GameInfo', round_num):
        ...
        if ((len(game_info.player_info.shop_pets) > 0) and tier2_dict[game_info.player_info.shop_pets[shop_pet_ranking[0]].type] < 3):
            ...
            # check whether the worst pet on your team is already a top 3 pet

            # worst pet is not part of top 3
            if tier2_dict[worst_pet.type] >= 3:
                if game_info.player_info.coins >= 2:
                    # sell the worst pet
                    print(f"before sell 501 length of pet shop: {len(game_info.player_info.shop_pets)}", flush = True)
                    bot_battle.sell_pet(worst_pet)
                    team_size -= 1
                    print(f"after sell 504 length of pet shop: {len(game_info.player_info.shop_pets)}", flush = True)

                    # after this move, update game info
                    print(f"507 length of pet shop: {len(game_info.player_info.shop_pets)}", flush = True)
                    print(f"508 length of our pets: {len(game_info.player_info.pets)}", flush = True)
                    game_info = bot_battle.get_game_info()
                    print(f"510 length of pet shop: {len(game_info.player_info.shop_pets)}", flush = True)
                    print(f"511 length of our pets: {len(game_info.player_info.pets)}", flush = True)

                    # buy the best pet in the shop
                    # buy the top 3 pet
                    print(f"index 0 shop_pet_ranking: {shop_pet_ranking[0]}", flush = True)
                    print(f"length of pet shop: {len(game_info.player_info.shop_pets)}", flush = True)
                    if (attempt_buy_pet(game_info, game_info.player_info.shop_pets[shop_pet_ranking[0]], False) == 0):
                        team_size += 1
                        return
                    # else:
            ...
            # worst pet is part of top 3
            else:
                worst_pet_index = team_pet_ranking[-1]
                upgrade = check_upgradable_b4_selling(game_info, game_info.player_info.pets, worst_pet_index)
                ...
                if game_info.player_info.coins >= 3:
                    if upgrade is not None:
                        ...
                        # buy the best food for the best pet
                        if (attempt_buy_food(game_info, shop_pet_ranking[0], team_pet_ranking[0]) == 0):
                            return
    ...
    def order_pets(game_info, pets):
        ...
        if peacock_index is not None and kangaroo_index is not None:
            ...
            if (peacock_index + 1) != kangaroo_index:
                ...
                if (move_made is True) and ((peacock_index + 1) < 5):
                    game_info = bot_battle.get_game_info()
                if ((peacock_index + 1) < 5):
    ...
    def check_upgradable_b4_selling(game_info, pets, pet_to_sell_index):
        ...
        for pet_index, pet in enumerate(pets):
            if (pet is not None) and (pet_to_sell is not None):
                if pet.level == 3 or pet_to_sell.level == 3:
                    continue
